...
do
	...
	do
		...
		do
    		# for ((i=0; i<${#line[@]}; i++))
    		# do
      	 	# 	echo "Value at index $i: ${line[$i]}"  { from: { column: 0, row: 0 }, to: { column: 1, row: 1 }, value: 5 },
    		# done
    		ri=$(char_to_int ${line[1]})
    		echo "{from:{column:"$c",row:"$r"}, to: { column:"$((c+1))", row:"$ri"}, value:"${line[0]}"},">>$json_file
    		#echo "{from:{" "\""${columns[$c]}${rows[$r]}"\", \"target\": \""${columns[$c+1]}${line[1]}"\", \"value\":"${line[0]}"}," >>$json_file
		done
	done
done
